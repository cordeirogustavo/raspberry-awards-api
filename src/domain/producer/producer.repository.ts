    ...
    return await this.databaseConnectionProvider.getAll(
      `WITH ranked_wins AS (
        ...
        winners_with_intervals AS (
          ...
          INNER JOIN ranked_wins following_award 
              ON previous_award.producers = following_award.producers
              AND previous_award.row_num + 1 = following_award.row_num
          WHERE following_award.win_year - previous_award.win_year > 0
        ),
